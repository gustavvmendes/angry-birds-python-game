...
while running:
    ...
    if mouse_pressed and level.number_of_birds > 0:
        ...
        if time.time()*1000 - t1 > BIRD_WAIT_TIME and level.number_of_birds > 0:
 ...
 for pig in pigs:
    ...
    pig_angle = pig.body.angle
    angle_degrees = math.degrees(pig_angle)
    ...
    FPS = 50
    clock.tick(FPS)
   pygame.display.set_caption(FPS_LABEL + str(clock.get_fps()))
